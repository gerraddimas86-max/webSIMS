...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .topbar-divider { color: var(--light); }
        ...
        .form-card { background: var(--surface); border: 1px solid var(--light); border-radius: 1rem; padding: 2rem; max-width: 680px; opacity: 0; animation: fadeUp .5s ease .1s forwards; }
        .form-title { font-family: 'Syne', sans-serif; font-size: 1.1rem; font-weight: 700; margin-bottom: .35rem; }
        .form-subtitle { font-size: .85rem; color: var(--mid); margin-bottom: 2rem; }
        .form-group { margin-bottom: 1.25rem; }
        .form-label { display: block; font-size: .8rem; font-weight: 600; color: #444; margin-bottom: .5rem; letter-spacing: .02em; text-transform: uppercase; }
        .form-input { width: 100%; padding: .75rem 1rem; background: var(--bg); border: 1.5px solid var(--light); border-radius: .75rem; font-family: 'Mulish', sans-serif; font-size: .9rem; color: var(--dark); outline: none; transition: border-color .2s, background .2s, box-shadow .2s; }
        .form-input:focus { background: #fff; border-color: var(--dark); box-shadow: 0 0 0 3px rgba(20,20,20,.06); }
        .form-input::placeholder { color: #bbb; }
        .form-input.error { border-color: #ef4444; }
        .error-msg { font-size: .78rem; color: #ef4444; margin-top: .35rem; }
        textarea.form-input { resize: vertical; min-height: 120px; }
        .form-footer { display: flex; align-items: center; gap: 1rem; margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid var(--light); }
        .btn-primary { display: inline-flex; align-items: center; gap: .5rem; padding: .75rem 1.5rem; background: var(--dark); color: #fff; border: none; border-radius: .75rem; font-family: 'Mulish', sans-serif; font-size: .875rem; font-weight: 600; cursor: pointer; transition: background .15s, transform .1s; }
        .btn-primary:hover { background: #333; transform: translateY(-1px); }
        .btn-secondary { display: inline-flex; align-items: center; padding: .75rem 1.5rem; background: transparent; border: 1.5px solid var(--light); border-radius: .75rem; font-family: 'Mulish', sans-serif; font-size: .875rem; font-weight: 600; color: var(--mid); text-decoration: none; transition: border-color .15s, color .15s; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('admin.events.create') }}" class="nav-item active">
...
<div class="main">
    <div class="topbar">
        <div class="topbar-left">
            ...
            <span class="topbar-title">Buat Event</span>
        </div>
    ...
    <div class="page">
        <div class="form-card">
            <div class="form-title">Event Baru</div>
            <div class="form-subtitle">Isi detail event yang akan diselenggarakan</div>

            @if($errors->any())
                <div style="margin-bottom:1.5rem;background:#fef2f2;border:1px solid #fecaca;border-radius:.75rem;padding:.9rem 1.1rem;font-size:.85rem;color:#dc2626;">
                    <ul style="list-style:disc inside;display:flex;flex-direction:column;gap:.2rem;">
            ...
            <form method="POST" action="{{ route('admin.events.store') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="event_date" value="{{ old('event_date') }}"
                        class="form-input {{ $errors->has('event_date') ? 'error' : '' }}"
                        min="{{ date('Y-m-d') }}">
